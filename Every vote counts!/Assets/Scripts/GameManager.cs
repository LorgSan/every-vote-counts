...
public class GameManager : MonoBehaviour
{
    ...
    void CreateBlank() //this thing creates the ballot itself
     {
        ...
        ballotPanel = canvas1.transform.GetChild(0).gameObject; //saving the gameobject that's gonna be moved
        //Button = ballotPanel.transform.GetChild(0).gameObject;
        Button = canvas1.transform.GetChild(2).gameObject; //saving the vutton to move separately from the ballot panel
        endingPanel = canvas1.transform.GetChild(1).gameObject; //same with the text in the end of the game
    ...
    void ButtonMove() //this function moves the button if the first vote (called in the input checker)
    {
        ...
        Vector3 firstButtonPos = new Vector3 (Button.transform.position.x, 0f, 0f);
